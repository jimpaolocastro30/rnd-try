...
(function (window){
	...
	var FWDBuyButton = function(
		...
		this.init = function(){
			...
			if(FWDEVPUtils.isIEAndLessThen9 || self.isMobile_bl){
		...
		this.buyClickHandler = function(){
			location.href = "http://codecanyon.net/item/easy-video-player/7152281?ref=FWDesign";
		}
		...
		this.scrollHandler = function(){
			var scrOfssets = FWDEVPUtils.getScrollOffsets();
			if(scrOfssets.y < 120){
				self.showHello();
			}
		}
		...
		this.buyAnimBackCompleteHandler = function(){
			if(FWDEVPUtils.isIEAndLessThen9 || self.isMobile_bl) return;
			self.showHello();
		};
    ...
    FWDBuyButton.setPrototype = function(){
    	FWDBuyButton.prototype = new FWDEVPDisplayObject("div");
    };
